...
> `映射`矩阵 $W_Q$、$W_K$、$W_V$ 的更新进化过程  

`映射`矩阵 $W_Q$、$W_K$、$W_V$ 是一组可学习的参数（一堆随机初始化、然后慢慢被数据训练出来的数字），它没有固定规则，是模型自己学出来的投影规则。
- 初始化：一开始**完全是随机数**
- 前向传播：$Q = XW_Q, K = XW_K, V = XW_V$，得到 Q、K、V，算注意力，算损失
- 反向传播：损失告诉 $W_Q$、$W_K$、$W_V$：“你现在的值不好，导致预测错了，你要往某个方向微调。”
- 梯度下降更新：$W_Q$、$W_K$、$W_V$  里的每个数字都被一点点修改
- 重复几万步：慢慢从随机数，变成能把 X 投影成有用 Q、K、V 的矩阵
...
## Dropout 
### 为什么要dropout/正则化？
**本质就是：**防止模型 “死记硬背”，逼它学真正规律  
- 模型太大、参数太多 → 它能把训练集里的每一个细节都背下来
- 但背下来不代表学会规律 → 换一句没见过的话就崩
- 正则化 = 给学习加点 “干扰”，不让它死记硬背
- Dropout 就是最常用、最简单的一种正则化

### 目标
正则化的终极目的：泛化  
- 正则化 ： 牺牲一点点训练集精度，换取模型在陌生句子上的表现。
- Dropout ： 随机关掉神经元，让模型不能死记硬背，只能学规律。

### 为什么模型会 “死记硬背”？
当模型参数、超参数太多时，模型记忆力极强
- 会 “死记硬背”，而不是 “真正学习”。
- 它完全可以不理解语义，只记模式
- 过拟合（overfitting）

### Dropout 怎么防止死记硬背？
训练时，随机把一部分神经元 “关掉”（置 0）  
比如 dropout=0.1：
- 每次 forward，随机 10% 输出变 0
- 模型每次看到的输入都不完全一样
- 它没法依赖某几个固定神经元
- 只能去学更通用、更稳定的特征

### minimind模型中容易出现“死记硬背”的地方
1. Transformer容易出现：
  - 对某些 token 分配极高权重
  - 形成 “固定依赖”
  - 换个句子结构就失效  
attn_dropout：防止注意力 “盯死某几个词”  
  - 随机把一些注意力权重抹掉
  - 模型不敢只盯某几个词
  - 必须学会更全面的语义关系
